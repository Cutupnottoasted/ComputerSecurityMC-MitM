...
def extract_packet_details(packet, packet_number):
    ...
    while cur_layer:
        ...
        if hasattr(cur_layer, 'fields_desc'):
            for field in cur_layer.fields_desc:
                ...
                if layer_name == '802.11' and field_name == 'subtype':
                    if field_value not in subtypes:
                        subtypes[field_value] = 1
                    else:
                        subtypes[field_value] += 1

        if 'EAPOL' in layer_name:
            print(packet_number)
        if layer_name == 'EAPOL':
            nonce = extract_nonce(cur_layer.load)
            layer_fields['nonce'] = nonce

        layer_details[f'{layer_name}'] = layer_fields
    ...
    if packet.haslayer(Dot11):
        ...
        seq_field = dot11_layer.SC
        if seq_field != None:
            seq_num = seq_field >> 4
            info_logger.info(f'Sequence Number: {seq_num}')
    return packet.time
...
def process_pcap(pcap_file, block_traffic=False):
    ...
    def process_packet(packet, packet_number):
        nonlocal block_traffic, prev_capture, time_since  # Use the block_traffic flag from the outer function
        ...
        if Dot11 in packet: 
            if not prev_capture:
                prev_capture = extract_packet_details(packet, packet_number)
            elif prev_capture:
                cur_time = extract_packet_details(packet, packet_number)
                if cur_time != None:
                    time_since = (cur_time - prev_capture)
                    info_logger.info(f'Time at packet transmission {packet_number}: {time_since} Seconds')
...
def main():
    ...
    if subtypes:
        for type, count in subtypes.items():
            print(f'Subtype {type}: occcured {count}')
